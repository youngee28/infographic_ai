...
export function InfographicChatPanel({ sessionId, analysisData, isAnalyzing }: InfographicChatPanelProps) {
  ...
  const [isAnalysisSummaryExpanded, setIsAnalysisSummaryExpanded] = useState(false);
  ...
  const summarySections = useMemo(
    () =>
      (analysisData?.summaries ?? [])
        .map((summary) => {
          const lines = summary.lines?.map((line) => line.text.trim()).filter(Boolean) ?? [];

          return {
            title: summary.title?.trim() || "요약",
            lines,
          };
        })
  ...
  const issueItems = useMemo(() => {
    if (!analysisData?.issues) return [];

    if (Array.isArray(analysisData.issues)) {
      return analysisData.issues.map((issue) => issue.text.trim()).filter(Boolean);
    }
  ...
  const keywordItems = analysisData?.keywords ?? [];
  const hasAnalysisSummary = Boolean(analysisData?.title || summarySections.length || keywordItems.length || issueItems.length);
...
위 테이블 데이터와 작업 내역을 바탕으로 정보 밀도가 높은 인포그래픽 또는 데이터 비주얼을 생성하세요. 제목, 핵심 지표 강조, 비교 구조, 읽는 순서를 반영한 결과를 우선합니다.`;
  ...
  return (
    <div className="h-full min-h-0 flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-hidden p-5 md:p-6 lg:p-7 w-full">
        <ImageChatTimeline
          messages={messages}
        ...
        <div className="space-y-2.5">
          ...
          {hasAnalysisSummary && (
            <div className="rounded-2xl border border-gray-200/80 bg-white p-3 shadow-sm">
              <div className={`flex justify-between gap-3 ${isAnalysisSummaryExpanded ? "items-start" : "items-center"}`}>
                <div>
                  <p className="text-[10.5px] font-semibold uppercase tracking-[0.16em] text-gray-400">분석 요약</p>
                  {isAnalysisSummaryExpanded && (
                    <p className="mt-1 text-[12px] leading-relaxed text-gray-500">
                      분석 결과를 보면서 인포그래픽 브리프를 바로 다듬을 수 있습니다.
                    </p>
                  )}
                </div>
                <button
                  type="button"
                  onClick={() => setIsAnalysisSummaryExpanded((previous) => !previous)}
                  aria-expanded={isAnalysisSummaryExpanded}
                  className="inline-flex shrink-0 items-center rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 text-[11px] font-medium text-gray-600 transition-colors hover:border-gray-300 hover:text-gray-800"
                >
                  {isAnalysisSummaryExpanded ? "접기" : "펼치기"}
                </button>
              </div>
                ...
                <div className="mt-3 space-y-2.5">
                  {analysisData?.title && (
                    <div className="rounded-xl border border-blue-100 bg-blue-50/70 px-3 py-2.5">
                      <p className="text-[10.5px] font-semibold uppercase tracking-[0.12em] text-blue-600">제목</p>
                      <p className="mt-1 text-[13px] font-semibold leading-snug text-gray-800">{analysisData.title}</p>
                    </div>
                  )}

                  {summarySections.length > 0 && (
                    <div className="rounded-xl border border-gray-200/80 bg-gray-50/70 px-3 py-2.5">
                      <p className="text-[10.5px] font-semibold uppercase tracking-[0.12em] text-gray-500">요약</p>
                      <div className="mt-2 space-y-2">
                        {summarySections.map((summary, index) => (
                          <div key={`${summary.title}-${index}`} className="space-y-1">
                            <p className="text-[11px] font-semibold text-gray-600">{summary.title}</p>
                            <ul className="space-y-1">
                              {summary.lines.map((line, lineIndex) => (
                                <li
                                  key={`${summary.title}-${index}-${lineIndex}`}
                                  className="flex items-start gap-1.5 text-[12px] leading-relaxed text-gray-700"
                                >
                  ...
                  {keywordItems.length > 0 && (
                    <div className="rounded-xl border border-gray-200/80 bg-gray-50/70 px-3 py-2.5">
                      <p className="text-[10.5px] font-semibold uppercase tracking-[0.12em] text-gray-500">키워드</p>
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {keywordItems.map((keyword, index) => (
                          <span
                            key={`${keyword}-${index}`}
                            className="rounded-md border border-gray-200/80 bg-white px-2.5 py-1 text-[11.5px] font-medium text-gray-600"
                          >
                            #{keyword}
                          </span>
                        ))}
                  ...
                  {issueItems.length > 0 && (
                    <div className="rounded-xl border border-rose-100 bg-rose-50/70 px-3 py-2.5">
                      <p className="text-[10.5px] font-semibold uppercase tracking-[0.12em] text-rose-600">주의 포인트</p>
                      <ul className="mt-2 space-y-1.5">
                        {issueItems.map((issue, index) => (
                          <li key={`${issue}-${index}`} className="flex items-start gap-1.5 text-[12px] leading-relaxed text-rose-800">
