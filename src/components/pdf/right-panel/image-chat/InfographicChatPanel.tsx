...
import { getAnalysisTitle, getLegacyKeywordFallback, getVisualizationPrompt } from "@/lib/analysis-selectors";
import { buildInfographicContext, extractGeneratedImageResult, getSelectedLayoutPlan } from "@/lib/infographic-generation";
...
const buildDerivedPrompt = (analysisData?: AnalysisData | null) => {
  ...
  if (activeLayoutPlan) {
    const sectionSummary = activeLayoutPlan.sections
      ...
      .slice(0, 4)
      .join(", ");
    return `${getAnalysisTitle(analysisData, "이 데이터")}를 ${activeLayoutPlan.aspectRatio} 비율의 dashboard 레이아웃으로 구성해줘.${sectionSummary ? ` 섹션은 ${sectionSummary} 중심으로 유지해줘.` : ""} 색감과 분위기는 이후 스타일 요청을 반영하되, 레이아웃 구조와 차트 배치는 유지해줘.`;
  }

  const keywordText = getLegacyKeywordFallback(analysisData).slice(0, 3).join(", ");
  return `${getAnalysisTitle(analysisData, "이 데이터")}를 바탕으로 핵심 수치와 비교 포인트가 잘 보이는 세로형 인포그래픽으로 구성해줘.${keywordText ? ` ${keywordText}를 우선 강조해줘.` : ""}`;
};
...
const getGeneratedPrompt = (analysisData?: AnalysisData | null) => {
  ...
  return analysisData.generatedInfographicPrompt?.trim() || getVisualizationPrompt(analysisData) || buildDerivedPrompt(analysisData);
};
...
const getActivePrompt = (analysisData?: AnalysisData | null) => {
  ...
  return analysisData.infographicPrompt?.trim() || getVisualizationPrompt(analysisData) || getGeneratedPrompt(analysisData);
};
  ...
  const persistPrompt = useCallback(
    async (nextPrompt: string, options?: { toastMessage?: string }) => {
      ...
      const nextAnalysisData = analysisData
        ? {
            ...analysisData,
            visualizationBrief: analysisData.visualizationBrief
              ? {
                  ...analysisData.visualizationBrief,
        ...
        if (session) {
          ...
          await store.saveSession({
            ...
            analysisData: sessionAnalysisData
                ? {
                    ...sessionAnalysisData,
                    visualizationBrief: sessionAnalysisData.visualizationBrief
                      ? {
                          ...sessionAnalysisData.visualizationBrief,
                          prompt: normalizedPrompt,
                        }
                      : sessionAnalysisData.visualizationBrief,
...
` : ""}최신 사용자 요청: ${userContent}
        ...
        ${hasLayoutPlan
          ? "위 앱이 계산한 layoutPlan을 최우선으로 따라 인포그래픽을 생성하세요. 섹션 순서, 차트 유형, KPI 블록, 정보 비중 정책은 유지하고, 최신 사용자 요청과 선택 속성은 스타일/강조/세부 연출 조정으로 반영하세요."
        ...
        const { generatedImageDataUrl, generatedText } = extractGeneratedImageResult(imageResult);
  ...
  useEffect(() => {
    ...
    const signature = `${sessionKey}:${getAnalysisTitle(analysisData) ?? ""}:${activePrompt}`;
  ...
  }, [activePrompt, analysisData, handleGenerate, isGenerating, isPendingAnalysis, messages.length, sessionKey]);
